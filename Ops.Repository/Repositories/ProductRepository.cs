...
namespace Ops.Repository.Repositories
{
    public class ProductRepository : GenericRepository<Product>,IProductRepository
    {
        ...
        public async Task<Product> GetProductDetailsById(int id)
        {
            return await _context.Products.Include(x=>x.Comments).ThenInclude(x=>x.Customer).Include(x => x.Stock).Include(x=>x.ProductFeature).Include(x => x.SizeProducts).ThenInclude(x => x.Size).Include(x => x.Photos).Include(x => x.ColorProducts).ThenInclude(x => x.Color).FirstOrDefaultAsync(x => x.Id == id);
        }
    }
}
